...
ull mmi(ull n) { return mpo(n, MOD - 2); }
...
void getph(int n, ull pre[], ull po[]) {
    ...
    for (int i = 2; i <= n; i++)
        pre[i] =
            (pre[i - 1] % MOD + (str[i - 1] % MOD * po[i - 1] % MOD) % MOD) %
            MOD;
    return;
}
...
void getsh(int n, ull suf[], ull po[]) {
    ...
    for (int i = n - 2, j = 2; i >= 0 && j <= n; i--, j++)
        suf[j] =
            (suf[j - 1] % MOD + (str[i] % MOD * po[j - 1] % MOD) % MOD) % MOD;
    return;
}
...
bool qr(Query q, int n, ull pre[], ull suf[], ull po[]) {
    ...
    ull rhlr =
        ((suf[n - L] - suf[n - R - 1] + MOD) % MOD * mmi(po[n - R - 1]) % MOD) %
        MOD;
    if (hlr == rhlr) {
        if (isp(L, R) == true)
            return true;
    ...
    while (scanf("%s", str) != EOF) {
        ...
        while (now < n) {
            ...
            for (int i = 0, j = now; j < n; i++, j++) {
                if (qr(Query(i, j), n, pre, suf, po) &&
                    qr(Query(i, ((i + j) / 2)), n, pre, suf, po)) {
                    cnt++;
                }
            }
            printf("%d ", cnt);
