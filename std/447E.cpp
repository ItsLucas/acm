...
void pushdown(int i, int l, int r) {
    ...
    add_fib(rson, mid + 1, r,
            get_fib(segtree[i].f1, segtree[i].f2, mid + 1 - l + 1),
...
void update(int i, int l, int r, int a, int b) {
    ...
    pushdown(i, l, r);
    if (a <= mid)
        update(lson, l, mid, a, b);
    if (b > mid)
        update(rson, mid + 1, r, a, b);
    pushup(i);
}
...
int query(int i, int l, int r, int a, int b) {
    ...
    int ans = 0;
    if (a <= mid)
        ans = (ans + query(lson, l, mid, a, b)) % mod;
    if (mid < b)
        ans = (ans + query(rson, mid + 1, r, a, b)) % mod;
    return ans;
}
